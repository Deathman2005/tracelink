...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setApiError(
        err.response?.data?.message || 'Failed to create account. Please check inputs and try again.'
      );
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2 text-center lg:text-left">
        <h3 className="text-2xl font-bold tracking-tight text-text-primary">Create your account</h3>
        <p className="text-sm text-text-secondary">Start sharing links and tracking assets with intelligence.</p>
      </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`w-full rounded-lg border px-10 py-3 text-sm text-text-primary bg-surface transition-colors focus:border-accent-blue focus:ring-2 focus:ring-accent-blue-light focus:outline-none ${
                nameError ? 'border-danger' : 'border-input-border'
              }`}
            />
          </div>
          {nameError && <p className="text-[11px] text-danger font-medium">{nameError}</p>}
        </div>
